...
{
	public class Sliding_TabStrip : LinearLayout
	{
		
		private const int DEFAULT_BUTTOM_BORDER_THICKNESS_DIPS = 2;
		private const byte DEFAULT_BUTTOM_BORDER_COLOR_ALPHA = 0X26;
		private const int SELECTED_INDICATOR_THICKNESS_DIPS = 8;
		private int[] INDICATOR_COLORS = { 0x19A319, 0x0000FC };   //滚轮颜色
		...
		//Constructor
			public Sliding_TabStrip(Context context) : this(context, null)
			{}
		public Sliding_TabStrip(Context context, IAttributeSet attrs) : base(context, attrs)
		{
			...
			mDefaultBottomBorderColor = SetColorAlpha(themeForeGround, DEFAULT_BUTTOM_BORDER_COLOR_ALPHA);

			mDefaultTabColorizer = new SimpleTabColorizer();
			mDefaultTabColorizer.IndicatorColors = INDICATOR_COLORS;
			mDefaultTabColorizer.DividerColors = DIVIDER_COLORS;

			mBottomBorderThickness = (int)(DEFAULT_BUTTOM_BORDER_THICKNESS_DIPS * density);
			mBottomBorderPaint = new Paint();
			mBottomBorderPaint.Color = GetColorFormInteger(0xC5C5C5); //Gray
		...
		private Color GetColorFormInteger(int color)
		{
		...
		protected override void OnDraw(Canvas canvas)
		{
			...
			if (tabCount > 0)
			{
				...
				mSelectedIndicatorPaint.Color = GetColorFormInteger(color);
				...
				for (int i = 0; i < ChildCount; i++)
				{
					...
					mDividerPaint.Color = GetColorFormInteger(tabColorizer.GetDividerColor(i));
